...
RC openTreeScan(BTreeHandle *tree, BT_ScanHandle **handle) {
    ...
    *b = temp;
    ...
    void sortKeysAndElements(int *sortedKeys, int **sortedElements, int totalKeys) {
        int i = 0;
        switch (i) {
            case 0:
                do {
                    ...
                    switch (j) {
                        case 0:
                            for (; j < totalKeys - i - 1; j++) {
                                if (sortedKeys[j] > sortedKeys[j + 1]) {
                                    swap(&sortedKeys[j], &sortedKeys[j + 1]);
                                    swap(&sortedElements[j][0], &sortedElements[j + 1][0]);
                                    swap(&sortedElements[j][1], &sortedElements[j + 1][1]);
                                }
                            }
                            // No break statement needed since it falls through to the next case
                        default:
