...
void testBlockMapReading(void) {
    ...
    ASSERT_TRUE(getBlockPos(&fh) == 4, "It did not move to the last block");

    // Test reading the next block from position 4 should fail as it does not exist
    ASSERT_TRUE(readNextBlock(&fh, ph) != RC_OK, "Non-existing next block");

    // Test reading the previous block from position 4, should succeed
    TEST_CHECK(readPreviousBlock(&fh, ph));
    ASSERT_TRUE(getBlockPos(&fh) == 3, "It did not move to the previous block");
    ...
    ASSERT_TRUE(getBlockPos(&fh) == 1, "It did not move to the next block after the first block");
